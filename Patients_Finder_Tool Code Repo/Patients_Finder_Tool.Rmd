...
- Get the data from Covid period onwards to remain the intergrity of the data pattern
...
### Calculate the proportion of patients waiting under 1 year by Specialty

```{r proportion under 1 year by specialty, echo = FALSE, warning=FALSE, message=FALSE}
...
### Plot the proportion of patients waiting under 1 year by Specialty

```{r plot proportion, echo = FALSE, warning=FALSE, message=FALSE}
...
### The proportion of patients waiting under 1 year for the whole dataset
...
### Caculate and plot Average waiting time for particular Specialty
...
```{r predictors relationship plot, eval = FALSE}
...
## Observe the potential predictors by PPP code
...
Average_surgery_length_ppp_category %>%
  kable("html", caption = "Average Surgery Length by PPP Category") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"),
                full_width = FALSE, 
                font_size = 14) %>%
  column_spec(1, bold = TRUE, border_right = TRUE) %>%  
...
surgery_metrics_wide %>%
  ...
  GGally::ggparcoord(
    columns = 3:5,  # Only select the numeric columns 
