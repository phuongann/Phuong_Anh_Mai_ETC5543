...
## Segmentation using PPPCategory group
 
```{r}
surgery_top_ppp <- surgery_patients %>% 
  group_by(PPPCategory) %>%
  summarise(CaseCount = n()) %>% 
  arrange(desc(CaseCount)) %>%
  slice_head(n = 1)

surgery_segment_ppp <-  surgery_patients[surgery_patients$PPPCategory %in% surgery_top_ppp$PPPCategory, ]
```
...
set.seed(123)
# Training and Testing sets
surgery_segment_ppp_split <- initial_split(surgery_segment_ppp,
                                        2/3,
                                        strata = WaitingListSpecialtyDesc)

 train_data <- training(surgery_segment_ppp_split)
 test_data <- testing(surgery_segment_ppp_split)
...
# Filter only available level in test data
test_data <- test_data %>%
  filter(
    AgeGroupAtDateOfRemoval %in% age_levels,
    CurrentPriorityCode %in% priority_levels,
    WaitingListSpecialtyDesc %in% specialty_levels,
    TreatmentCampusName %in% treament_campus_levels,
    PlannedStayTypeDescription %in% planned_stay_levels,
    AdminCategory %in% admin_levels
  ) %>%
  mutate(
    AgeGroupAtDateOfRemoval = factor(AgeGroupAtDateOfRemoval, levels = age_levels),
    CurrentPriorityCode = factor(CurrentPriorityCode, levels = priority_levels),
    WaitingListSpecialtyDesc = factor(WaitingListSpecialtyDesc, levels = specialty_levels),
    PPPCategory = factor(PPPCategory, levels = unique(train_data$PPPCategory)),
    TreatmentCampusName = factor(TreatmentCampusName, levels = treament_campus_levels),
    PlannedStayTypeDescription = factor(PlannedStayTypeDescription, levels = planned_stay_levels),
    AdminCategory = factor(AdminCategory, levels = admin_levels)
  )
...
```{r}
# Find the minimum number of observations in any specialty and set v (as the old approach is not enough data to process)
min_obs <- surgery_segment_ppp %>%
  count(WaitingListSpecialtyDesc) %>%
  summarise(min_n = min(n)) %>%
  pull(min_n)

# Set v to be less than or equal to min_obs
v_folds <- min(min_obs, 5)

set.seed(123)
# Create cross-validation folds 
surgery_folds <- vfold_cv(train_data, v = v_folds, strata = WaitingListSpecialtyDesc)
...
surgery_predictions <- surgery_models %>%
  mutate(
    ...
    test_matrices = map(splits, ~{
      test_data <- assessment(.x)
      x_test <- model.matrix(log(SurgeryLength)~ AgeGroupAtDateOfRemoval + CurrentPriorityCode +
                                WaitingListSpecialtyDesc + 
                                PlannedStayTypeDescription +
                                #PPPCategory +
                                #PPPDesc + 
...
surgery_metrics_wide %>% 
  select(id, model, rmse, mae, rsq) %>% 
  # GGally::ggparcoord(columns = 3:6,    # Need adjustment, recheck
